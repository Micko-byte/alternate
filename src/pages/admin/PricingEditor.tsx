...
const AI_COST_PER_CREDIT_KES = 17; // a checked Standard try-on, including redos; HD and Studio cost less per credit

/** What ALTERNATE keeps from one credit sold at this price, after VAT, Paystack, the store's 20% and AI cost. */
function keepPerCredit(pricePerCredit: number) {
  return pricePerCredit / (1 + VAT) - pricePerCredit * (PAYSTACK + STORE_SHARE) - AI_COST_PER_CREDIT_KES;
}

function Keep({ price, credits }: { price: number; credits: number }) {
  const value = credits > 0 ? keepPerCredit(price / credits) : 0;
...
export function PricingEditor() {
  ...
  useEffect(() => {
    setPackRows(Object.fromEntries((packs.data ?? []).map((p) => [p.id, { name: p.name, price_kes: String(p.price_kes), credits: String(p.credits), is_active: p.is_active }])));
  }, [packs.data]);
    ...
    <section className="grid gap-6 border border-rule bg-surface p-6">
      <div className="grid gap-1">
        ...
        <p className="text-muted">
          "Keep / credit" estimates what ALTERNATE keeps from each credit if the buyer came through a store: after 16% VAT, 1.5% Paystack, the store's 20% and about {kes(AI_COST_PER_CREDIT_KES)} of AI per Standard try-on. Red means too thin.
        </p>
      </div>
      ...
      <div className="grid gap-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[620px] text-left text-[13px]">
            <thead className="label"><tr><th className="py-2">Name</th><th>Price</th><th>Credits</th><th>Keep / credit</th><th>Listed</th><th /></tr></thead>
            <tbody>
              {(packs.data ?? []).map((p) => {
                ...
                return (
                  <tr key={p.id} className="border-t border-rule">
                    ...
                    <td className="pr-2"><Keep price={Number(r.price_kes)} credits={Number(r.credits)} /></td>
                    <td className="pr-2"><input type="checkbox" className="h-4 w-4 accent-ink" checked={r.is_active as boolean} onChange={(e) => set("is_active", e.target.checked)} aria-label="Listed" /></td>
                    <td>
                      <Button size="sm" variant="outline" loading={busy === p.id} onClick={() => save(p.id, () => supabase.from("credit_packs").update({ name: String(r.name).trim(), price_kes: Number(r.price_kes), credits: Number(r.credits), is_active: !!r.is_active }).eq("id", p.id), "credit-packs")}>Save</Button>
                    </td>
                  </tr>
                );
              })}
